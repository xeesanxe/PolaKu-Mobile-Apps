...
const REQUIRED_FIELDS = [
  "sleepHours", "symptoms", "mealFrequency", "stressLevel", "mood",
];

/**
 * Decodes a base64-encoded audio string into a raw Buffer.
 * @param {string} base64Audio Base64-encoded audio data.
 * @return {Buffer} Decoded audio bytes.
 */
...
/**
 * Transcribes an audio buffer to Indonesian text via Groq Whisper.
 * @param {Buffer} audioBuffer Raw audio bytes (m4a).
...
/**
 * Sends the transcript to Groq LLM and extracts the 5 check-in fields.
 * @param {string} transcript Transcribed voice note text.
 * @return {Promise<Object>} Parsed { extractedFields, confidence } object.
 */
async function extractFieldsFromTranscript(transcript) {
  const systemPrompt = `Kamu adalah asisten ekstraksi data kesehatan ` +
    `untuk mahasiswa.
Dari transkrip curhat suara berikut, ekstrak HANYA 5 field ini jika ` +
    `disebutkan secara eksplisit atau bisa disimpulkan wajar:
- sleepHours (number, jam tidur semalam)
- symptoms (array of string, pilih HANYA dari daftar ini: ` +
...
Field yang TIDAK disebutkan atau tidak bisa disimpulkan, JANGAN diisi ` +
    `(jangan menebak paksa).
Abaikan bagian cerita yang tidak relevan dengan 5 field di atas.
Balas HANYA dalam format JSON: {"extractedFields": {...hanya field ` +
    `yang berhasil diekstrak...}, "confidence": "high"|"medium"|"low"}`;
...
exports.voiceExtract = onCall(
    ...
    async (request) => {
      ...
      try {
        ...
        if (!transcript || transcript.trim().length === 0) {
          throw new HttpsError(
              "invalid-argument",
              "Rekaman tidak dapat diproses, coba rekam ulang atau gunakan " +
              "mode pilihan.",
          );
        }
      } catch (err) {
      ...
      const missingFields = REQUIRED_FIELDS.filter(
          (field) => extractedFields[field] === undefined ||
